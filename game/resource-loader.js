...
// TODO(manifest): tie these explicit paths into config/build-manifest.json once modules are externalized.

(function() {
    'use strict';

    const warnInlineManifests = () => {
        const bundle = window.__ALL_MANIFESTS || {
            assetManifest: window.__ASSET_MANIFEST
        };
        if (bundle) {
            window.ManifestUtils?.warnInlineAssetCount?.(bundle);
        }
    };
