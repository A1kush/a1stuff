...
class CharacterController {
    constructor(name, model, isPlayer = false) {
        ...
        // Auto combat settings for follower AI
        this.autoAttackRange = 8;
    }
    ...
    handleFollowerAutoCombat() {
        if (this.isPlayer || !window.enemies || window.enemies.length === 0) return;

        const attackRange = this.autoAttackRange || 8;
        let nearest = null;
        let minDist = attackRange;

        for (const enemy of window.enemies) {
            if (!enemy || enemy.isDead || !enemy.model) continue;
...
class Enemy {
    ...
    die() {
        ...
        // Phase 21: Quest tracking - notify Quest Manager
        if (window.QuestManager && typeof window.QuestManager.onEnemyKilled === 'function') {
            window.QuestManager.onEnemyKilled();
        }
    }
}
...
function init() {
    ...
    // Spawn the mission board near spawn so players immediately see new content
    if (window.MissionBoardSystem && typeof window.MissionBoardSystem.spawn === 'function') {
        window.MissionBoardSystem.spawn('mission', new THREE.Vector3(3, 0, 8), 0);
        window.MissionBoardSystem.spawn('quest', new THREE.Vector3(-3, 0, 8), 0);
    }
