...
class ProjectileManager3D {
    ...
    spawnProjectile(skillData, sourcePos, targetPos, options = {}) {
        ...
        // Quick Feature 2: Character projectile variations
        const charId = skillData.characterId || options.characterId || 'A1';
        const element = skillData.element || 'PHYSICAL';
        let geometry, material;

        // Character-specific shapes
        if (charId === 'A1') {
            geometry = new THREE.BoxGeometry(0.3, 0.1, 0.8); // Sword slash
        } else if (charId === 'MISSY') {
            geometry = new THREE.OctahedronGeometry(0.2, 0); // Magic orb
        } else if (charId === 'UNIQUE') {
            geometry = new THREE.ConeGeometry(0.15, 0.6, 8); // Plasma bolt
        } else {
            geometry = new THREE.SphereGeometry(0.15, 8, 8); // Default
        }

        // Element-specific visuals
        if (element === 'ICE' || element === 'CRYO') {
            geometry = new THREE.IcosahedronGeometry(0.2, 0); // Icy crystal
        } else if (element === 'LIGHTNING' || element === 'ELECTRO') {
            geometry = new THREE.TetrahedronGeometry(0.2, 0); // Lightning shape
        }

        material = new THREE.MeshBasicMaterial({
            color: color,
            emissive: color,
            emissiveIntensity: element === 'FIRE' || element === 'LIGHTNING' ? 1.0 : 0.5,
            transparent: true,
            opacity: 0.9
        });
        ...
        if (charId === 'A1') mesh.rotateX(Math.PI / 2);
    ...
    update(dt, enemies) {
        for (let i = this.projectiles.length - 1; i >= 0; i--) {
            ...
            // Quick Feature 2: Rotate projectiles for visual effect
            if (proj.rotationSpeed) {
                proj.mesh.rotation.z += proj.rotationSpeed;
                proj.mesh.rotation.y += proj.rotationSpeed * 0.5;
            }
            ...
            // Check collisions with destructible objects and environmental interactions
            if (window.EnvSystem && window.EnvSystem.objects && proj.damage) {
                for (const obj of window.EnvSystem.objects) {
                    if (obj.userData && obj.userData.isDestructible) {
                        // Calculate object size for collision detection
                        const scale = obj.userData.scale || { x: 1, y: 1, z: 1 };
                        const objSize = Math.max(scale.x || 1, scale.z || 1) / 2;
                        const dist = proj.mesh.position.distanceTo(obj.position);

                        if (dist < objSize + 1) {
                            // Handle environmental elemental interactions (Phase 18.2)
                            if (proj.skillData && proj.skillData.element && window.EnvSystem.handleElementalInteraction) {
                                const element = proj.skillData.element;
                                window.EnvSystem.handleElementalInteraction(obj, element, proj.mesh.position);
                            }

                            // Damage destructible objects (rocks, trees, etc.)
                            if (obj.userData.hp > 0 && window.EnvSystem.damage) {
...
class CombatEngine3D {
    constructor(scene) {
        ...
        // Phase 18.3: Reaction Chain System
        this.reactionChain = 0;
        this.lastReactionTime = 0;
        this.reactionChainTimeout = 5000; // 5 seconds to maintain chain
        this.reactionChainMultiplier = 1.0;
        this.createReactionChainUI();
    ...
    // Phase 18.3: Create Reaction Chain UI
    createReactionChainUI() {
        // Check if UI already exists
        if (document.getElementById('reaction-chain-ui')) return;

        const ui = document.createElement('div');
        ui.id = 'reaction-chain-ui';
        ui.innerHTML = `
            <div class="reaction-chain-label">REACTION CHAIN</div>
            <div class="reaction-chain-count" id="reaction-chain-count">0</div>
            <div class="reaction-chain-multiplier" id="reaction-chain-multiplier">x1.0</div>
        `;
        ui.style.cssText = `
            position: fixed;
            top: 120px;
        ...
        style.textContent = `
            ...
            .reaction-chain-count {
                font-size: 24px;
                font-weight: bold;
                color: #00ffff;
                text-shadow: 0 0 10px rgba(0, 255, 255, 0.8);
            }
            .reaction-chain-multiplier {
                font-size: 16px;
                color: #ffff00;
                margin-top: 4px;
                font-weight: bold;
    ...
    // Update reaction chain counter
    updateReactionChain(reactionType) {
        if (!reactionType) return;

        const now = Date.now();

        // Reset chain if too much time passed
        if (now - this.lastReactionTime > this.reactionChainTimeout) {
            this.reactionChain = 0;
        }
        ...
        // Calculate multiplier (1.0 + 0.1 per chain, max 3.0x)
        this.reactionChainMultiplier = Math.min(1.0 + (this.reactionChain * 0.1), 3.0);

        // Update UI
        const ui = document.getElementById('reaction-chain-ui');
        const countEl = document.getElementById('reaction-chain-count');
        const multEl = document.getElementById('reaction-chain-multiplier');

        if (ui && countEl && multEl) {
            ui.style.display = 'block';
            countEl.textContent = this.reactionChain;
            multEl.textContent = `x${this.reactionChainMultiplier.toFixed(1)}`;

            // Pulse animation
            ui.style.animation = 'none';
            setTimeout(() => {
                ui.style.animation = 'reactionChainPulse 0.3s ease-out';
            }, 10);
        }

        // Auto-hide after timeout
        clearTimeout(this.reactionChainUITimeout);
        this.reactionChainUITimeout = setTimeout(() => {
            if (Date.now() - this.lastReactionTime > this.reactionChainTimeout) {
                const ui = document.getElementById('reaction-chain-ui');
                if (ui) {
    ...
    checkElementalReaction(target, attackElement, damage) {
        ...
        // Status Check Helper
        const hasStatus = (type) => window.StatusSystem.hasStatus(target, type);
        ...
        // --- PHASE 18: ADVANCED REACTIONS ---

        // 1. OVERLOAD (Fire + Lightning) -> AoE Explosion + Knockback
        if ((attackElement === 'FIRE' && hasStatus('conductive')) ||
            (attackElement === 'LIGHTNING' && hasStatus('burn'))) {
            finalDamage *= 2.5;
            reaction = "OVERLOAD!";

            // AoE Explosion Logic
            if (window.combatEngine && window.combatEngine.projectileManager && target.model) {
                window.combatEngine.projectileManager.spawnExplosion({
                    element: 'FIRE',
                    damage: damage * 0.5,
                    id: 'overload_blast'
                }, target.model.position, 8);
            }

            removeStatus('conductive');
            removeStatus('burn');
        }

        // 2. SUPERCONDUCT (Ice + Lightning) -> Phys Res Down (Debuff)
        else if ((attackElement === 'ICE' && hasStatus('conductive')) ||
                 (attackElement === 'LIGHTNING' && hasStatus('frozen'))) {
            finalDamage *= 1.5;
            reaction = "SUPERCONDUCT!";

            // Apply Vulnerability Debuff
            if (target.defenseMultiplier === undefined) target.defenseMultiplier = 1.0;
            target.defenseMultiplier = 0.5;

            setTimeout(() => { if(target) target.defenseMultiplier = 1.0; }, 5000);

            removeStatus('conductive');
            removeStatus('frozen');
        }

        // 3. SWIRL (Wind + Element) -> Spread Status
        else if (attackElement === 'WIND' || attackElement === 'ANEMO') {
            const spreadStatuses = ['burn', 'wet', 'frozen', 'conductive'];
            const foundStatus = spreadStatuses.find(s => hasStatus(s));
            ...
            if (foundStatus) {
                ...
                if (window.enemies) {
                    window.enemies.forEach(e => {
                        if (e !== target && e.model && e.model.position.distanceTo(target.model.position) < 10) {
                            window.StatusSystem.applyStatus(e, foundStatus, 5.0);
                            window.createFloater(`Spread: ${foundStatus}`, e.model.position, '#00ff88');
                        }
                    });
                }
            }
        ...
        if (!reaction) {
            ...
            if (attackElement === 'FIRE' || attackElement === 'PYRO') window.StatusSystem.applyStatus(target, 'burn', 3.0);
            if (attackElement === 'ICE' || attackElement === 'CRYO') window.StatusSystem.applyStatus(target, 'frozen', 2.0);
            if (attackElement === 'LIGHTNING' || attackElement === 'ELECTRO') window.StatusSystem.applyStatus(target, 'conductive', 2.0);
            // ANEMO/WIND doesn't apply status by itself, only spreads existing elements
        }
    ...
    processHit(target, skillData) {
        ...
        // Phase 18.3: Apply reaction chain multiplier
        if (reactionResult.reaction) {
            this.updateReactionChain(reactionResult.reaction);
            dmg *= this.reactionChainMultiplier;
        }
        ...
        if (reactionResult.reaction && target.model) {
            ...
            else if (reactionResult.reaction === 'OVERLOAD!') color = '#ff4400'; // Red-orange
            else if (reactionResult.reaction === 'SUPERCONDUCT!') color = '#00ffff'; // Cyan
            else if (reactionResult.reaction && reactionResult.reaction.startsWith('SWIRL:')) {
                // Color based on spread element
                if (reactionResult.reaction.includes('WATER')) color = '#00aaff';
                else if (reactionResult.reaction.includes('FIRE')) color = '#ff4400';
                else if (reactionResult.reaction.includes('ICE')) color = '#00ffff';
                else if (reactionResult.reaction.includes('LIGHTNING')) color = '#ffff00';
                else color = '#ffffff';
            }
...
function updateAutoAttack(dt) {
    ...
    // Collect loot (gold + item pickups)
    if (window.Game && window.Game.scene) {
        window.Game.scene.children.forEach(child => {
            if (!child.userData || child.userData.collected) return;

            const isGold = child.userData.type === 'gold';
            const isLoot = child.userData.isLoot === true;
            if (!isGold && !isLoot) return;

            const d = leader.model.position.distanceTo(child.position);
            const pickupDistance = isGold ? 3 : 3.5;
            const magnetDistance = 30;

            if (d < pickupDistance) {
                child.userData.collected = true;

                if (isGold && window.gameState) {
                ...
                if (isLoot && typeof window.collectWorldItem === 'function') {
                    window.collectWorldItem(child.userData.item);
                }

                window.Game.scene.remove(child);
            } else if (d < magnetDistance) {
                const dir = new THREE.Vector3().subVectors(leader.model.position, child.position).normalize();
                const speed = isGold ? 15 : 18;
                child.position.add(dir.multiplyScalar(speed * dt));
            }
        });
    }
