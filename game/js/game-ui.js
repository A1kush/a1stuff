...
function handleInput() {
    ...
    player.input.x = (keys['a'] || keys['arrowleft']) ? 1 : (keys['d'] || keys['arrowright']) ? -1 : 0; // Fixed: swapped left/right arrows
    ...
    show: (npcName, text) => {
        ...
        if (!document.getElementById('dialogue-box')) {
            ...
            div.style.cssText = `
                ...
                width: 80%; max-width: 600px; background: rgba(0,0,0,0.85);
                border: 2px solid #00e5ff; color: white; padding: 20px;
                font-family: 'Courier New', monospace; font-size: 16px;
                border-radius: 10px; z-index: 2000; display: none;
            `;
            div.innerHTML = `<h3 style="margin:0 0 10px 0; color:#00e5ff;" id="dia-name"></h3><p id="dia-text" style="margin:0;"></p><div style="text-align:right; font-size:12px; color:#ccc; margin-top:10px;">[Press Interact to Close]</div>`;
            document.body.appendChild(div);
            DialogueUI.element = div;
        }

        const box = document.getElementById('dialogue-box');
        const nameEl = document.getElementById('dia-name');
        const textEl = document.getElementById('dia-text');
        ...
        // Typewriter Effect
        let i = 0;
        clearInterval(DialogueUI.typingTimer);
        DialogueUI.typingTimer = setInterval(() => {
            textEl.innerText += text.charAt(i);
            i++;
            if (i >= text.length) clearInterval(DialogueUI.typingTimer);
        }, 30);
    },
...
// Integrate into UniversalInteractionSystem (Modify existing in game-integration.js or override here)
// We hook into the 'act' button logic
