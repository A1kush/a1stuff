...
DEFAULT = {"size": (128, 128), "frames": 12, "fps": 14, "pivot": "center"}
...
def draw_uplift(frame_index, frames, size):
    ...
    if frame_index == frames - 2:
        draw.ellipse(
            (w // 2 - 8, y1 - 8, w // 2 + 8, y1 + 8), outline=A1K["purple"], width=2
        )
    return img
...
def draw_fx_dark_aura_idle(frame_index, frames, size):
    ...
    # thin neon rim
    draw.ellipse(
        (cx - pulse - 2, cy - pulse - 2, cx + pulse + 2, cy + pulse + 2),
        outline=A1K["cyan"],
        width=1,
    )
    draw.ellipse(
        (cx - pulse - 4, cy - pulse - 4, cx + pulse + 4, cy + pulse + 4),
        outline=A1K["purple"],
        width=1,
    )
...
def draw_fx_rage_surge_burst(frame_index, frames, size):
    ...
    # purple halo
    draw.ellipse(
        (cx - r - 4, cy - r - 4, cx + r + 4, cy + r + 4), outline=A1K["purple"], width=1
    )
    # red danger specks
    for i in range(6):
...
def draw_fx_shockwave_heavy(frame_index, frames, size):
    ...
    # purple halo (fainter)
    draw.ellipse(
        (cx - r - 3, cy - r - 3, cx + r + 3, cy + r + 3), outline=A1K["purple"], width=1
    )
...
def draw_fx_shadow_portal_spawn(frame_index, frames, size):
    ...
    draw.ellipse(
        (cx - r - 8, cy - r - 8, cx + r + 8, cy + r + 8), outline=A1K["cyan"], width=1
    )
    ...
    for i in range(6):
        ...
        draw.line(
            (x2, y2, x2 + int(math.cos(ang) * 6), y2 + int(math.sin(ang) * 6)),
            fill=A1K["purple"],
            width=1,
        )
    # falling sparks
